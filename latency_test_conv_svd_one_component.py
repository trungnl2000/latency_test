...
from custom_op.conv_svd_one_component import wrap_convSVD_one_component
 
class ConvLayer_svd_one_component(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, SVD_var, active, current_index, forward_time, backward_time):
        super(ConvLayer_svd_one_component, self).__init__()
        self.conv = wrap_convSVD_one_component(nn.Conv2d(in_channels, out_channels, kernel_size), 
...
def main(**kwargs):
    ...
    ####################################################
    print("_____SVD conv one component_____")
    ...
    for i in range(number_of_iteration):
        ...
        active = True
        # Create layer
        conv_layer_svd = ConvLayer_svd_one_component(in_channels, out_channels=out_channels, kernel_size=kernel_size, SVD_var=SVD_var, 
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_file", type=str, default="latency_test_conv_svd", help="Output file to write results.")
